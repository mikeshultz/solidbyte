...
def get_default_banner(contracts=[], variables={}):
    return \
        "Solidbyte Console ({})\n" \
        "------------------------------\n" \
...
class SolidbyteConsole(code.InteractiveConsole):
    def __init__(self, _locals=None, filename="<console>",
    ...
    def interact(self, banner=None, exitmsg="bye!"):
        super(SolidbyteConsole, self).interact(banner or get_default_banner(self.contracts, self.locals), exitmsg)
